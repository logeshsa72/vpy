...
import { TextInput, CheckBox, DropdownInput, LongTextInput, MultiSelectDropdown } from "../../../Inputs"
...
export default function Form() {
  ...
  const { data: singleData, isFetching: isSingleFetching, isLoading: isSingleLoading } =  useGetEmployeeCategoryByIdQuery(id, { skip: !id });
...
// }
  ...
  const syncFormWithDb = useCallback(
    (data) => {
      ...
      setEmpId(data?.empId?data.empId:"")
  ...
  const data = {
   name, code, companyId: secureLocalStorage.getItem(sessionStorage.getItem("sessionId") + "userCompanyId"), active, id, ProductUomPriceDetails,empId
  }
            ...
            <div className='mr-1 md:ml-2'>
              <fieldset className='frame my-1'>
                ...
                <div className='grid grid-cols-2 my-2'>
                  ...
                  <TextInput name="Employee Name" type="text" value={name} setValue={setName} required={true} readOnly={readOnly} disabled={(childRecord.current > 0)} />
                   <TextInput name="Employee Id" type="text" value={empId} setValue={setEmpId} required={true} readOnly={readOnly} disabled={(childRecord.current > 0)} />
