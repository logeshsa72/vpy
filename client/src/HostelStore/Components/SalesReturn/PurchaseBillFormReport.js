...
const PurchaseBillFormReport = ({
  ...
  useEffect(() => { setCurrentPageNumber(1) }, [dataPerPage, searchDocId, searchBillDate, searchSupplierName,  ])
  ...
  const { data: allData, isLoading, isFetching } = useGetSalesReturnQuery({ params: { branchId, ...searchFields, pagination: true, dataPerPage, pageNumber: currentPageNumber } });
  ...
  return (
    <div id='registrationFormReport' className="flex flex-col w-full h-[95%] overflow-auto">{console.log(allData,"alldataaa")}
      <div className="md:flex md:items-center md:justify-between page-heading p-1">
        <div className="heading text-center md:mx-10">Sales Return Report </div>
