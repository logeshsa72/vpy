...
import { useGetPurchaseBillByIdQuery, useGetPurchaseBillQuery } from '../../../redux/services/PurchaseBillService';
...
export default function Form() {
  ...
  const { data: allData, isLoading, isFetching } = useGetPurchaseReturnQuery({ params: { branchId }, searchParams: searchValue });
  const { data: singleData, isFetching: isSingleFetching, isLoading: isSingleLoading } = useGetPurchaseReturnByIdQuery(id, { skip: !id });
console.log(singleData,'si');
  ...
  const { data: purchaseOrderDetails, isFetching: isPurchaseOrderDetailsFetching, isLoading: isPurchaseOrderDetailsLoading } = useGetPurchaseBillByIdQuery(purchaseOrderId, { skip: !purchaseOrderId });

  const { data: purchaseBillData } = useGetPurchaseBillQuery({ params: { branchId } });
  ...
  const syncFormWithDb = useCallback(
    (data) => {
      ...
      setPoReturnItems(data?.PoReturnItems ? data.PoReturnItems : []);
      setDueDate(data?.dueDate ? moment.utc(data?.dueDate).format("YYYY-MM-DD") : "");
      setPurchaseOrderId(data?.purchaseBillId ? data?.purchaseBillId : "")
  ...
  const data = {
    ...
    supplierId,
    dueDate,
    purchaseBillId: purchaseOrderId,
    address,
    place,
    poReturnItems: poReturnItems.filter(item => item.qty != 0),
  ...
  const validateData = (data) => {
    if (data.supplierId && data?.poReturnItems?.length > 0) {
      return true;
    }
    return false;
  }
