...
} from '../../../redux/services/ProductCategoryServices'

const MODEL = "Product Category Master";
...
export default function Form() {
  ...
  const { data: allData, isLoading, isFetching } = useGetProductCategoryQuery({params, searchParams:searchValue});
  const { data: singleData, isFetching: isSingleFetching, isLoading: isSingleLoading } = useGetProductCategoryByIdQuery(id, {skip: !id});

  const [addData] = useAddProductCategoryMutation();
  const [updateData] = useUpdateProductCategoryMutation();
  const [removeData] = useDeleteProductCategoryMutation();

 

  const syncFormWithDb = useCallback(
    (data) => {
     console.log(id,"iddd",data,"data",singleData?.data,"sigledataaaa")
